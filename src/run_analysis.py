...
import os
import pandas as pd  # Fix 1: Properly import pandas
import numpy as np
from src.data_processing import load_and_clean_data, calculate_log_returns
from src.bayesian_modeling import build_and_sample_change_point_model, extract_summary_metrics
...
def main():
    ...
    try:
        ...
        # Fix 2: Pull the correct numerical mean prices directly from the summary table
        mu1_val = summary.loc["mu_1", "mean"]
        mu2_val = summary.loc["mu_2", "mean"]

        # Calculate percentage shift using numbers, not dates
